...
public class ProductoController {
   ...
   //     producto.setImagen(UUID.randomUUID().toString() + "-" + file.filename()
          ...
          //      .replace(":", "")
        //        .replace("\\", ""));
      //  return file.transferTo(new File(path + producto.getImagen())).then(service.save(producto)).map(p -> ResponseEntity
   ...
   // }
    ...
    public Mono<ResponseEntity<Producto>> upload(@PathVariable String id, @RequestPart FilePart file) {
        return service.findById(id).flatMap(p -> {
                    p.setImagen(UUID.randomUUID().toString() + "-" + file.filename()
                            .replace(" ", "")
                            .replace(":", "")
                            .replace("\\", ""));
                    return file.transferTo(new File(path + p.getImagen())).then(service.save(p));
